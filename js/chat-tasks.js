// ═══════════════════════════════════════════
// CHAT MODULE — Real-time via Socket.io
// ═══════════════════════════════════════════

// ─── Load project chat (initial fetch) ───────────────────
async function loadProjectChat() {
  ...
  SocketManager.joinProject(State.currentProject._id);
  // Mark as read
  if (window._socket?.connected) window._socket.emit('message:read', { projectId: State.currentProject._id });
}

// ─── Load global chat (initial fetch) ────────────────────
async function loadGlobalChat() {
  const { ok, data } = await api('/chat/global?limit=80');
  if (!ok) return;
  State.globalMessages = data.messages || [];
  renderMessages('global-chat-list', State.globalMessages, null);
...
function renderMessages(containerId, messages, projectId) {
  ...
  container.innerHTML = messages.map(msg => buildMessageHTML(msg, me, projectId)).join('');
}

// ─── Append single message (real-time, no flicker) ───────
function appendMessage(containerId, msg, projectId) {
  const container = document.getElementById(containerId);
  if (!container) return;
  ...
  div.innerHTML = buildMessageHTML(msg, me, projectId);
  const node = div.firstElementChild;
  if (node) container.appendChild(node);
}

// ─── Build message HTML ───────────────────────────────────
function buildMessageHTML(msg, me, projectId) {
  const isOwn = msg.sender?._id === me?._id || msg.sender === me?._id;
  const senderName = msg.sender?.name || 'Unknown';
  if (msg.isDeleted) {
    return `<div class="msg-group" id="msg-${msg._id}"><div class="msg-bubble" style="font-style:italic;color:var(--text-muted);opacity:0.6">🗑️ Message deleted</div></div>`;
  }
  return `
    <div class="msg-group" id="msg-${msg._id}">
      ${!isOwn ? `<div class="msg-sender">
        <div class="msg-avatar" data-user-id="${msg.sender?._id || ''}">${avatarLetters(senderName)}</div>
      ...
      <div style="display:flex;${isOwn ? 'justify-content:flex-end' : ''}">
        <div class="msg-bubble ${isOwn ? 'own' : ''}">
          ...
          ${msg.messageType === 'image' ? `<img src="${fileUrl(msg.fileUrl)}" style="max-width:200px;border-radius:8px;margin-bottom:4px">` : ''}
          ${msg.messageType === 'file' ? `<a href="${fileUrl(msg.fileUrl)}" target="_blank" style="color:var(--accent)">📎 ${msg.fileName || 'File'}</a>` : ''}
          ${msg.content ? `<span class="msg-content-text">${escHtml(msg.content)}</span>` : ''}
          ${msg.tags?.length ? msg.tags.map(t => `<span class="msg-tag">${t.replace(/_/g,' ')}</span>`).join('') : ''}
          ${isOwn ? `<div style="text-align:right;margin-top:4px"><span style="font-size:10px;color:var(--text-muted)">${fmtTime(msg.createdAt)}${msg.isEdited ? ' · edited' : ''}</span></div>` : ''}
        </div>
      </div>
      ${msg.reactions?.length ? `<div class="msg-reactions">${msg.reactions.map(r => `<button class="msg-reaction-btn">${r.emoji} ${r.users.length}</button>`).join('')}</div>` : ''}
    </div>
  `;
}
...
// ─── Project chat send (via Socket, fallback to REST) ─────
...
if (chatTextarea) {
  ...
  chatTextarea.addEventListener('input', function() {
    ...
    if (State.currentProject) {
      SocketManager.startTyping(State.currentProject._id);
      clearTimeout(_typingTimer);
      _typingTimer = setTimeout(() => SocketManager.stopTyping(State.currentProject._id), 1500);
    }
  });
}
...
async function sendProjectMessage() {
  ...
  SocketManager.stopTyping(State.currentProject._id);

  // Try socket first (real-time, no REST round-trip)
  const sent = window._socket?.connected && window._socket.emit('message:send', {
  ...
  if (!sent) {
    ...
    if (ok) {
      ...
      appendMessage('chat-messages-list', data.message, State.currentProject._id);
      scrollChatToBottom('chat-messages-list');
...
// ─── Global chat send (via Socket, fallback to REST) ──────
...
async function sendGlobalMessage() {
  ...
  const sent = window._socket?.connected && window._socket.emit('global:send', { content });

  if (!sent) {
    const { ok, data } = await api('/chat/global', { method: 'POST', body: { content } });
    if (ok) {
      State.globalMessages.push(data.message);
      appendMessage('global-chat-list', data.message, null);
      scrollChatToBottom('global-chat-list');
    } else showToast('Failed to send', 'error');
  }
}
...
async function uploadChatFile(file) {
  ...
  if (ok) { showToast('File sent!', 'success'); loadProjectChat(); }
  else showToast('Upload failed', 'error');
}

// ─── Initialise chat tags & file upload (called after DOM ready) ─
function initChatTagsAndUpload() {
  // Tags toggle
  document.querySelectorAll('.chat-tag').forEach(btn => {
    // Remove any stale listener by cloning
    const fresh = btn.cloneNode(true);
    btn.parentNode.replaceChild(fresh, btn);
    fresh.addEventListener('click', () => fresh.classList.toggle('selected'));
  });
  ...
  if (fileInput) {
    fileInput.addEventListener('change', function() {
      if (this.files[0]) uploadChatFile(this.files[0]);
...
function renderTasksBoard(tasks) {
  ...
  ['pending', 'in_progress', 'done'].forEach(status => {
    ...
    col.innerHTML = groups[status].length
      ? groups[status].map(t => renderTaskCard(t)).join('')
      : `<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:13px">No tasks</div>`;
  });
}
    ...
    <div class="task-card card card-sm" style="margin-bottom:10px;cursor:pointer" onclick="openEditTask('${t._id}')">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-8">
          ${t.assignedTo ? `<div class="user-avatar" style="width:22px;height:22px;font-size:9px" data-user-id="${t.assignedTo._id || ''}">${avatarLetters(t.assignedTo.name || '')}</div>` : ''}
...
document.getElementById('form-create-task')?.addEventListener('submit', async (e) => {
  ...
  if (ok) { showToast('Task created!', 'success'); closeModal('modal-create-task'); e.target.reset(); loadTasks(); }
...
async function populateTaskProjectSelect() {
  ...
  if (sel) sel.innerHTML = '<option value="">No project (personal)</option>' + State.projects.map(p => `<option value="${p._id}">${p.name}</option>`).join('');
  if (selAssigned && State.teamMembers.length) selAssigned.innerHTML = '<option value="">Unassigned</option>' + State.teamMembers.map(u => `<option value="${u._id}">${u.name}</option>`).join('');
}
...
async function runSearch(q) {
  ...
  if (!totalResults) { resultsEl.innerHTML = `<div class="empty-state"><div class="empty-icon">🔍</div><div class="empty-title">No results for "${query}"</div></div>`; return; }
  ...
  sections.forEach(({ key, icon, label }) => {
    ...
    html += `<div class="card mb-16"><div class="card-header"><div class="card-title">${icon} ${label} <span class="badge badge-accent">${items.length}</span></div></div><div class="table-wrapper"><table class="table"><tbody>${items.map(item => `<tr><td class="primary">${item.term || item.keyword || item.campaign || item.content?.substring(0,60) || item.title || '—'}</td><td>${statusBadge(item.status || item.action || item.matchType || '')}</td><td style="font-size:11px;color:var(--text-muted)">${item.project?.name || ''}</td></tr>`).join('')}</tbody></table></div></div>`;
  });
