...
function renderMembersList(p) {
  ...
  membersEl.innerHTML = (p.members || []).map(m => {
    ...
    return `
    <div class="flex items-center gap-12" style="padding:12px 0;border-bottom:1px solid var(--border)" id="member-row-${uid}">
      <div class="user-avatar" style="width:36px;height:36px;font-size:14px">${avatarLetters(m.user?.name || '')}</div>
      <div style="flex:1">
        <div style="font-weight:600;font-size:14px">${m.user?.name || 'Unknown'}</div>
        <div style="font-size:12px;color:var(--text-muted)">${m.user?.email || ''}</div>
      ...
      ${canChangeRole ? `
        <div style="display:flex;align-items:center;gap:6px">
          <select id="role-select-${uid}" class="form-select" style="width:110px;font-size:12px;padding:4px 8px">
            <option value="owner"  ${m.role === 'owner'  ? 'selected' : ''}>Owner</option>
            <option value="editor" ${m.role === 'editor' ? 'selected' : ''}>Editor</option>
            <option value="viewer" ${m.role === 'viewer' ? 'selected' : ''}>Viewer</option>
          </select>
          <button class="btn btn-primary btn-sm" style="padding:4px 10px;font-size:12px"
            onclick="saveMemberRole('${uid}', '${m.role}')" title="Save role">✓</button>
          <button class="btn btn-ghost btn-sm btn-icon" onclick="removeMemberFromProject('${uid}')" title="Remove member">✕</button>
        </div>
      ` : `<span class="badge ${roleClass}">${m.role}</span>`}
    </div>`;
  }).join('');
...
// Called only when owner clicks ✓ — avoids re-render on every dropdown open/change
async function saveMemberRole(userId, currentRole) {
  const select = document.getElementById(`role-select-${userId}`);
  if (!select) return;
  const newRole = select.value;
  if (newRole === currentRole) { showToast('Role is already ' + newRole, 'info'); return; }
